...
class HeifEncoderTest {
    ...
    @Ignore("Robolectric does not reliably model MediaCodecList HEIF encoder discovery.")
    @Test
    fun heifAvailabilityProbeFindsEncoder() {
        HeifAvailability.resetCache()

        assertTrue(HeifAvailability.isWriteSupported())
    }

    @Ignore("Robolectric does not provide a real HEIF codec for HeifWriter encode output.")
    @Test
    fun heifEncodeWritesHeifContainer() = runBlocking {
        val result = encoder.encode(
            photoLikeBitmap(64, 64),
            EncodeFormat.HEIF,
            80,
