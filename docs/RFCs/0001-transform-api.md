...
| `webplossless` | `EncodeFormat.WEBP` (lossless)        | `quality` segment must be `q100`.  |
...
5. **Cross-field consistency** — `qauto` without `targetBytes`,
   `webplossless` with quality ≠ `q100`, `aspectLock` without `exact` →
   `MalformedUri`.
6. **Range checks** — quality in `1..100`, dimensions within DoS bounds.
7. **Source URI present and decodable** → otherwise `MissingSource`.
...
`.meta` file holding `(mime, params, callingUid, createdAtMs, lastAccessMs)`.
...
cap, evict by oldest `lastAccessMs` until under cap. Eviction is
synchronous on the writer's thread — simple, predictable, and avoids a
background thread that the OS might reap.

### 3.4 24-hour sweep

On every app launch, a `WorkManager`-free coroutine (we cannot add deps
and we already do this for `cacheDir/output/` in v1.0) walks
`transform-cache/` and deletes entries with `createdAtMs` older than 24 h.
This matches the privacy-policy statement: *"Cached copies of incoming and
processed images (auto-cleared after 24 hours)"*. Keeping the Transform API
under the same window means no privacy-policy edit is needed.
...
- **Breaking change** (renaming a token, changing a default, changing a
  field's meaning, retiring a format): requires a new path segment
  (`/v2/`) served *alongside* `/v1/` during a deprecation window.
- **Bugfix** (the URI parsed but produced wrong output): lands under
  `/v1/` and is announced in release notes. Golden-output tests (see §4.3)
  prevent accidental behavior drift during bugfix work.
...
3. **App**: continue serving `/vX/` for the full window. Add a one-shot
   ...
   developer attaching `adb logcat` sees the deprecation notice.
...
edited. A test that "needs" to be updated is a contract break in disguise.
...
| 2 | Result-bytes leak               | Output cached on disk after host consumes it                  | Cache lives in `cacheDir/` (per-app, private); filenames are SHA-256 hashes; LRU + 24 h sweep (§3.3, §3.4).            |
...
| Caching        | `cacheDir/transform-cache/`, SHA-256 keys, 200 MB LRU, 24 h sweep            |
